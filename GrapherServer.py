...
if not os.path.exists(os.path.join(os.getcwd(), 'data_vault.ini')):
    repo = os.path.split(sys.argv[0])[0]
    while 1:
        print 'Enter directory for data storage [%s]' % repo
        ans = raw_input('>>> ').strip()
        if ans != '':
            repo = ans
        if os.path.exists(repo):
            break
        print 'Directory not found.  Shall I create it? (y/[n])'
    ...
    with open('data_vault.ini', 'w') as f:
        f.write('[config]\nrepository = %s\n' % repo)
