...
import random
import numpy as np
from util.units import Value, Unit
...
def averagedScan(r,averages,noisy=True):
    """
    Turns an array of values, r, into a generator that will loop over r any number of times
    """
    def delay_gen():
        for average in range(averages):
            if noisy:
                print('average '+str(average+1))
            for x in r:
                yield x
    return delay_gen
